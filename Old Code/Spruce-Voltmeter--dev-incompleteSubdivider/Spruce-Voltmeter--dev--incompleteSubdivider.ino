 ...
 * 
 * This version uses MINIMAL serial so that OLED, SD, and everything works (usually).
 * May 8 2022
 * 
 * 
 * For custom fileName saved to SD card, switch comments in void setup
 * 
 * 
...
int aPinsLength = 3;
int SIGFIGS = 4;           // How many figs to print/save
...
void loop() {
  if (linesWritten < linesPerSave) {
    ...
    */
    ...
    if (printSerial) {
      formatMatrixSerialPrint(3, 6);
    }
    // save to SD card
    if (myFile) {
      formatMatrixSDPrint(3, 6);
    }
    else {
      if (printSerial) {
