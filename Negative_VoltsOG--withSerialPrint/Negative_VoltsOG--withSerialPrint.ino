...
void setup(void) {
  Serial.begin(9600);
  while(!Serial) {;} // wait for USB serial connection to stabilize to avoid reading noise
...
void loop(void) {
  ...
  /// Half AREF measurement: stabilize against OpAmp fluctuations  ///    

  dump = analogRead(A0); // discard first adc convertion
  delay(10); // delay for channel switching stabilization
